...
typedef CactusProgressCallback =
    void Function(double? progress, String statusMessage, bool isError);
...
class CactusInitParams {
  ...
  /// Absolute path to a pre-extracted model directory. When set, Cactus skips
  /// the Supabase registry lookup and the slug-derived documents path, and
  /// loads weights directly from this location.
  final String? modelPath;

  /// Quantization override used together with [modelPath]. Required for
  /// externally-provisioned models (e.g. int4-only HF weights) where Cactus
  /// can't query Supabase for the quantization value.
...
enum CompletionMode { local, hybrid }

enum TranscriptionProvider { whisper }
...
class STTInitParams {
  ...
  STTInitParams({required this.model});
}
