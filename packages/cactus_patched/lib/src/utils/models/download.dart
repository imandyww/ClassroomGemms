...
class DownloadService {
  ...
  static Future<bool> modelExists(String folderName, [String? basePath]) async {
    ...
    final modelsRootPath = _modelsRootPath(appDocDir.path, basePath);
    _assertSafeModelStoragePath(modelsRootPath, purpose: 'store model cache');
    final modelFolderPath = p.join(modelsRootPath, folderName);
  ...
  static Future<bool> downloadAndExtractModels(
    ...
    final modelsRootPath = _modelsRootPath(appDocDir.path, basePath);
    _assertSafeModelStoragePath(modelsRootPath, purpose: 'store model cache');
    final modelsDir = Directory(modelsRootPath);
  ...
  static Future<bool> _downloadAndExtractModel(
    ...
    final modelsRootPath = _modelsRootPath(appDocDir.path, basePath);
    _assertSafeModelStoragePath(modelsRootPath, purpose: 'store model cache');
    final modelFolderPath = p.join(modelsRootPath, folder);
    _assertSafeModelStoragePath(
      modelFolderPath,
      purpose: 'extract model files',
    );
    final modelFolder = Directory(modelFolderPath);
    ...
    final modelsDir = Directory(modelsRootPath);
    await modelsDir.create(recursive: true);
    final zipFilePath = p.join(modelsDir.path, filename);
    ...
    try {
      ...
      await for (final chunk in response) {
        ...
        if (contentLength > 0) {
          ...
          downloadProcessCallback?.call(
            progress,
            'Downloaded ${totalBytes ~/ (1024 * 1024)} MB...',
            false,
          );
      ...
      if (filename.toLowerCase().endsWith('.zip')) {
        ...
        final destinationPath = p.join(modelFolderPath, filename);
  ...
  static Future<void> _extractZipFile(
    ...
    _assertSafeModelStoragePath(extractToPath, purpose: 'extract model files');
    ...
    try {
      ...
      for (final file in archive) {
        ...
        if (file.isFile) {
          final extractedFilePath = _resolveOutputPath(
            extractToPath,
            relativePath,
          );
          if (extractedFilePath == null) continue;
          ...
          final dirPath = _resolveOutputPath(extractToPath, relativePath);
      ...
      for (final file in symbolicLinks) {
        ...
        if (relativePath.isNotEmpty) {
          final linkPath = _resolveOutputPath(extractToPath, relativePath);
          if (linkPath == null) continue;
          if (!_isValidSymlinkTarget(
            extractToPath,
            linkPath,
            file.symbolicLink!,
          )) {
...
String _modelsRootPath(String appDocPath, String? basePath) =>
    basePath ?? p.join(appDocPath, 'models');
...
String? _resolveOutputPath(String outputRoot, String relativePath) {
  ...
  final resolved = p.normalize(p.join(outputRoot, normalizedRelative));
